...
//mport { adminPayload } from 'src/auth/admin-payload.interface';
...
export class UserService {
  ...
  async reset(email): Promise<boolean> {
    ...
    const otp = reSending
      ...
      const newPass =  passwordGen.generate({
        length: 8,
        numbers: true,
        symbols : true,
        uppercase : true,
    ...
    // send th email out
    await this.mailService.send({
      subject: `Your one time PIN to reset your password : ${otp}`,
      template: 'pass-reset-otp.twig',
      templateParams: {
        otp: otp,
        pass : newPass,
      },
  ...
  async resetP(email,otp): Promise<boolean> {
    ...
    existingUser.code = otp;
    //.password = newPass;
    console.log("The new password ",newPass)
    // send th email out
    await this.mailService.send({
      subject: `Your new password : ${newPass}`,
      template: 'pass-reset.twig',
      templateParams: {
        pass : newPass
      },
      to: email,
    ...
    console.log("The newly hashed password");
    await usersRepo.save(existingUser);
