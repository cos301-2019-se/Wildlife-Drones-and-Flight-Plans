...
export class RangerService {
  ...
  async addRanger(long: number, lat: number, rangerID: number): Promise<boolean> {
    ...
    try {
      let rangerUser = await con.getRepository(User).findOne({ id: rangerID, jobType: 'ranger' });
  ...
  async updateRangerLocation(long: number, lat: number, rangerID: number): Promise<boolean> {
    ...
    try {
      let rangerUser = await con.getRepository(User).findOne({ id: rangerID, jobType: 'ranger' });
      let updateRanger = await con.getRepository(Ranger).findOne({ ranger: rangerUser })
      ...
      console.log(
        'Updated loaction of ranger with id: ' + updateRanger.id + ' and rangerid: ' + rangerUser.id,
      );
